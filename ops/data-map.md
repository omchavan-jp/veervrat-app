...
| **Database dumps** | Two places, and the difference matters. **(a)** `veervrat<env>backups`, Azure Blob, Central India — staging, private, encrypted. **(b)** A maintainer's machine, in India, holding the copy pulled out of Azure. Only (b) satisfies #131: (a) is in the same subscription as the database it protects. Each dump is a complete copy of every personal record the platform holds, AES-256 encrypted before it leaves the job, retained **30 days** in both places. Live for UAT since 2026-08-27 and for **prod since `prod-2026-08-30`** — the tag that created `veervratprodbackups`, since the job and its storage ship with the environment. ⚠️ The encryption key is per-environment and a prod dump will not open with the UAT key; both live in `~/.secrets/veervrat/` and in the shared password manager, because they are also in each environment's Key Vault — which is inside the subscription these copies exist to survive. See `openspec/changes/offsite-backup` |
| Sessions / cache | Azure Managed Redis, same region |
| Secrets | Azure Key Vault, per environment |
| Object storage | Azure Blob, **Central India** — same region as everything else, so this does not affect the residency claim the way Sentry does. Both environments (`veervratuatuploads`, `veervratproduploads` — the latter created by `prod-2026-08-24.1`). ⚠️ Prod ran the pre-#178 single-container layout until `prod-2026-08-30`, which brought it level with UAT. **Prod's account does hold at least one file** — an `experience` image written 2026-08-25 — so the write path has run there; an earlier note here said it never had, which the data contradicts. What has *not* been demonstrated on prod is the read-back: nobody holds a blob data-plane role on `veervratproduploads`, and fetching that image as a guest returns 404 **by design** (see the verified row below), so a guest-level check cannot tell a present blob from an absent one. Proving prod end-to-end needs a signed-in session that owns the containing log. ✅ **Exercised end-to-end on UAT.** Uploaded 2026-08-24; made private 2026-08-25 (#178). An image is now served by the api, which decides per request whether the viewer may see it — **visibility derives from the document containing the image**, so a chat image follows room membership and an experience image follows that log's own visibility (including guest access to a published one). Verified on UAT: uploader 200, anonymous 404, blob unreachable directly 404. Blog images stay public and cacheable, deliberately. See `documentation/22_Platform-Requirements.md` |
