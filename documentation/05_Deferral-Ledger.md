...
| 31 | ~~**Account-settings Section 5 — Vratmitra settings**: global-VM change/migration + Restart tour~~ | Item 32 (spec/26 §5) | Item 37 | ✅ Done (Item 37 — `removeGlobalVm(cascade keep\|unassign)` ends global + optionally journey assignments, pending approvals untouched, notifies outgoing VM via emailable VM_WITHDREW; "change" = remove + fresh global invite; `POST /users/me/restart-tour` sets `tourResetAt` without resetting onboarding; Settings Section 5 UI) |
...
| 33 | **`/onboarding/account-setup` form does not submit under Playwright** — neither the Continue button nor Enter fires the `complete-onboarding` request (no client validation error shown); real users are unaffected. E2E flow 1 completes onboarding via API instead. | Item 35 | Investigate the account-setup form's submit handler under automation; then assert the full UI onboarding in flow 1 | ⬜ Open |
| 34 | **Non-platform invite → signup → accept VM role** has no backfill: signup does not link a pending invitation to the new account (`invitations.inviteeId` stays null), grant the VRATMITRA role, or emit `INVITEE_JOINED_PLATFORM`. So an invited newcomer cannot accept (`vm_invitation.accept` needs `isVm` + `inviteeId === user.id`). E2E flow 4 skips the accept step. Related to #8 (invite lifecycle). | Item 35 (found) | Invite→account linking + role grant + INVITEE_JOINED_PLATFORM on signup | ⬜ Open |
