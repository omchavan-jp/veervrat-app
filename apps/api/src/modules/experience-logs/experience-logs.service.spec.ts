...
describe('ExperienceLogsService', () => {
  describe('create', () => {
    it('creates a global draft for a VA (forced draft/only_me)', async () => {
      const repo = makeRepo();
      const service = new ExperienceLogsService(repo, makeJourneys());
    ...
    it('NEGATIVE: rejects an empty/invalid body', async () => {
      const service = new ExperienceLogsService(makeRepo(), makeJourneys());
    ...
    it('NEGATIVE: VA cannot create a journey-scoped entry on another VA\'s journey', async () => {
      ...
      const service = new ExperienceLogsService(makeRepo(), makeJourneys(otherJourney));
    ...
    it('creates a journey-scoped entry for the journey owner', async () => {
      const repo = makeRepo();
      const service = new ExperienceLogsService(repo, makeJourneys(ownJourneySlim));
