...
function makeService(parts: {
  ...
  s['usersRepository'] = {
    ...
    updateSettings: vi.fn().mockResolvedValue({
      ...
      profileVisibility: {}, notificationPrefs: {}, createdAt: new Date(), updatedAt: new Date(),
    }),
  ...
  s['authService'] = { forceLogout: vi.fn().mockResolvedValue(undefined), verifyPassword: vi.fn().mockResolvedValue(true), ...parts.auth };
  return { service, s };
}
...
describe('UsersService — settings', () => {
  it('updateSettings merges notificationPrefs and re-syncs the index', async () => {
    ...
    await service.updateSettings('u1', { profilePrivate: true, notificationPrefs: { JOURNEY_DORMANT: false } });
    const repo = s['usersRepository'] as Record<string, ReturnType<typeof vi.fn>>;
    expect(repo['updateSettings']).toHaveBeenCalled();
    expect((s['usersIndex'] as Record<string, ReturnType<typeof vi.fn>>)['upsert']).toHaveBeenCalled();
  });
});
...
describe('UsersService — anonymiseAccount', () => {
  it('replaces PII, force-logs-out, cancels invites, drops from index', async () => {
    ...
    expect((s['authService'] as Record<string, ReturnType<typeof vi.fn>>)['forceLogout']).toHaveBeenCalled();
    expect(repo['cancelPendingInvitations']).toHaveBeenCalled();
    expect((s['usersIndex'] as Record<string, ReturnType<typeof vi.fn>>)['remove']).toHaveBeenCalled();
  });
});

describe('UsersService — selfDelete', () => {
  it('anonymises when the password is correct', async () => {
    const { service, s } = makeService({ auth: { verifyPassword: vi.fn().mockResolvedValue(true) } });
    await service.selfDelete('u1', 'correct');
    expect((s['usersRepository'] as Record<string, ReturnType<typeof vi.fn>>)['anonymise']).toHaveBeenCalled();
  });
...
describe('parseNotificationPrefs', () => {
  ...
  it('drops unknown keys, non-booleans, and in-app-only events', () => {
    expect(parseNotificationPrefs({ NEW_FOLLOWER: false, BOGUS: true, JOURNEY_DORMANT: 'no' })).toEqual({});
  });
});
