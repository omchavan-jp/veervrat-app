...
describe('AdminPothiService', () => {
  it('NEGATIVE: non-admin cannot create a section', async () => {
    const { service } = make();
    await expect(service.create(VA, { sectionNumber: 1, titleEn: 'A' })).rejects.toBeInstanceOf(AccessDeniedException);
  });

  it('create with valid ordered shlokas', async () => {
    const { service, repo } = make();
    await service.create(ADMIN, { sectionNumber: 1, titleEn: 'A', shlokaIds: ['a', 'b'] });
    expect(repo.createPothiSection).toHaveBeenCalledWith(expect.objectContaining({ titleEn: 'A' }), ['a', 'b']);
  });
  ...
  it('update 404 when section missing', async () => {
    ...
    await expect(service.update(ADMIN, 'p1', { titleEn: 'B' })).rejects.toBeInstanceOf(EntityNotFoundException);
  });
});
