...
describe('content-editor disambiguateKeys', () => {
  it('returns a single key unchanged', () => {
    expect(disambiguateKeys(['a.b'], { insideDialog: false, routeSegments: [] })).toEqual(['a.b']);
  });
  ...
  it('narrows to the one key mentioning "modal" when clicked inside a dialog', () => {
    ...
    expect(disambiguateKeys(keys, { insideDialog: true, routeSegments: ['study'] })).toEqual([
      'study.browser.whyModal.title',
    ]);
  });
  ...
  it('filters out keys from an unrelated top-level namespace via route segments', () => {
    ...
    expect(disambiguateKeys(keys, { insideDialog: false, routeSegments: ['study'] })).toEqual([
      'study.why.title',
    ]);
  });
  ...
  it('falls back to the full set when filtering would remove everything', () => {
    ...
    expect(disambiguateKeys(keys, { insideDialog: true, routeSegments: ['study'] }).sort()).toEqual(
      keys.sort(),
    );
  });
});
