    ...
    <div className="mx-auto max-w-[680px]">
      ...
      <div className="mt-6 space-y-3">
        <Link href="/moderation/custom-erc" className="flex items-center gap-3 rounded-2xl border border-border bg-surface p-4 shadow-card transition-colors hover:border-accent/30">
          ...
          {pendingCount > 0 && <span className="rounded-full bg-accent px-2 py-0.5 font-mono text-[11px] font-semibold text-bg">{pendingCount}</span>}
          <ChevronRight className="h-4 w-4 shrink-0 text-muted" />
        </Link>

        {/* Reported comments — panel lands with a later moderation item; card shown for context. */}
        <div className="flex items-center gap-3 rounded-2xl border border-dashed border-border p-4 opacity-60">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[12px] text-muted">{t('comingSoon')}</div>
          </div>
        </div>
      </div>
    </div>
