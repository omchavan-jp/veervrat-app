...
export default function BlogDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <article>
      <h1 className="font-display text-[clamp(26px,3vw,36px)] font-medium leading-tight tracking-tight">
        {data.title}
      </h1>
      <div className="mt-2 text-[13px] text-muted">
        <Link href={`/u/${data.author.username}`} className="hover:text-accent">
      ...
      <section className="mt-10 border-t border-border pt-6">
        <h2 className="mb-4 text-[16px] font-medium">
          {t('comments', { count: data.comments.length })}
        </h2>
        ...
        <div className="space-y-3">
          {data.comments.map((c) => (
            <div
              key={c.id}
              className={`rounded-xl border border-border bg-surface p-3 ${c.isHidden ? 'opacity-60' : ''}`}
            >
              <div className="mb-1 flex items-center gap-2 text-[12px] text-muted">
                ...
                {c.isHidden && (
                  <span className="rounded-full bg-muted/15 px-2 py-0.5 text-[10px]">
                    {t('hidden')}
                  </span>
                )}
              </div>
                ...
                <div className="mt-1 flex gap-1 text-[11px] text-muted">
                  {(isBlogAuthor || isMod) && !c.isHidden && (
                    <Button
                      ...
                      disabled={hide.isPending && hide.variables === c.id}
                  ...
                  {canModerate(c) && (
                    <Button
                      ...
                      className="h-auto gap-1 px-2 py-1 text-[11px] hover:text-danger"
                    >
                  ...
                  {c.authorId !== user?.id && (
                    <Button
                      ...
                      disabled={report.isPending && report.variables === c.id}
                      className="h-auto gap-1 px-2 py-1 text-[11px] hover:text-warning"
                    >
