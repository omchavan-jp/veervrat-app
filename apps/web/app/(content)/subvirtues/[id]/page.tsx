...
export default function SubvirtueDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href={`/virtues/${data.virtue.id}`} className="inline-flex items-center gap-1 text-[13px] text-muted hover:text-accent">
        <ArrowLeft className="h-3.5 w-3.5" aria-hidden="true" /> <ContentText en={data.virtue.nameEn} mr={data.virtue.nameMr} />
      </Link>
      <div className="mt-3">
        <BilingualText en={data.nameEn} mr={data.nameMr} size="xl" />
      </div>
      <div className="mt-1 text-[13px] text-muted">{t('partOf', { virtue: locale === 'mr' && data.virtue.nameMr ? data.virtue.nameMr : data.virtue.nameEn })}</div>
        ...
        <section className="mt-8">
          ...
          <div className="flex flex-wrap gap-2">
            {data.weaknesses.map((w) => (
              <Link key={w.id} href={`/weaknesses/${w.id}`} className="rounded-full border border-border-strong px-3.5 py-1.5 transition-colors hover:border-accent">
