...
function ShlokaBlock({ s }: { s: Shloka }) {
  return (
    <div className="rounded-xl border border-border bg-bg p-4">
      ...
      {s.transliteration && (
        <div className="mt-1 text-[12px] italic text-muted">{s.transliteration}</div>
      )}
    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-2">
            <CmsInfoModal
              cmsKey="what-is-pothi"
              linkLabel={t('whatIsPothiLink')}
              fallbackTitle={t('whatIsPothiTitle')}
              fallbackBody={t('whatIsPothiBody')}
            />
          </div>
        </div>
        <div className="flex flex-wrap gap-1">
          <Button variant="link" size="sm" nativeButton={false} render={<Link href="/shlokas" />}>
            {t('seeMoreShlokas')}
          </Button>
          <Button variant="link" size="sm" nativeButton={false} render={<Link href="/resources" />}>
            {t('resources')}
          </Button>
        </div>
      </div>
            ...
            {(data ?? []).map((section) => (
              ...
              >
                <div className="mb-1 font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
                  {t('section', { n: section.sectionNumber })}
                </div>
                <BilingualText en={section.titleEn} mr={section.titleMr} size="lg" />
                {section.introText && (
                  <p className="mt-3 text-[14px] leading-relaxed text-muted">{section.introText}</p>
                )}
                {section.shlokas.length > 0 && (
                  <div className="mt-4 space-y-3">
                    {section.shlokas.map((s) => (
                      <ShlokaBlock key={s.id} s={s} />
                    ))}
                  </div>
                )}
                {section.congregationResponse && (
                  <p className="mt-3 text-[13px] italic text-muted">
                    {section.congregationResponse}
                  </p>
                )}
                {section.postShlokaCommentary && (
                  <p className="mt-3 text-[14px] leading-relaxed">{section.postShlokaCommentary}</p>
                )}
              </section>
            ))}
          </div>
