...
function ShlokaCard({ s, onOpen }: { s: Shloka; onOpen: () => void }) {
  return (
    <button onClick={onOpen} className="block w-full rounded-2xl border border-border bg-surface p-4 text-left shadow-card transition-colors hover:border-accent/30">
...
function ShlokaDetailBody({ data }: { data: ShlokaDetail }) {
  ...
  return (
    <div>
      ...
      {data.transliteration && <div className="mt-2 text-[13px] italic text-muted">{data.transliteration}</div>}
      {data.meaningEn && <div className="mt-3 text-[15px]">{data.meaningEn}</div>}
      {data.meaningMr && <div className="mt-1 font-deva text-[15px] text-muted">{data.meaningMr}</div>}
      {data.sourceCitation && <div className="mt-3 font-mono text-[11px] uppercase tracking-[0.1em] text-muted">{data.sourceCitation}</div>}
      {(data.formalTags.length > 0 || data.looseTags.length > 0) && (
        <div className="mt-4 border-t border-border pt-3">
          <div className="mb-2 font-mono text-[10px] uppercase tracking-[0.1em] text-muted">{t('tags')}</div>
          <div className="flex flex-wrap gap-1.5">
            {data.formalTags.filter((tg) => tg.name).map((tg) => (
              <span key={tg.entityId} className="rounded-full bg-accent/12 px-2.5 py-0.5 text-[12px] text-accent">{tg.name}</span>
            ))}
            {data.looseTags.map((lt) => (
              <span key={lt} className="rounded-full bg-muted/15 px-2.5 py-0.5 text-[12px] text-muted">{lt}</span>
            ))}
          </div>
        </div>
      )}
...
export default function ShlokasPage() {
  ...
  const list = useQuery({ queryKey: queryKeys.content.shlokas(), queryFn: () => contentApi.shlokas(), enabled: !searching });
  const search = useQuery({ queryKey: queryKeys.content.shlokaSearch(debounced), queryFn: () => contentApi.searchShlokas(debounced), enabled: searching });
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {items.map((s) => <ShlokaCard key={s.id} s={s} onOpen={() => setOpenId(s.id)} />)}
          </div>
        )}
      </div>
