...
export default function CmsPanel() {
  ...
  const startEdit = (p: CmsPage) =>
    setEditing({
      ...
      titleEn: p.titleEn,
      titleMr: p.titleMr ?? '',
      bodyEn: tiptapDocToText(p.bodyEn),
      bodyMr: tiptapDocToText(p.bodyMr),
    });
    ...
    <div className="mx-auto max-w-[680px]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button
          className="shrink-0"
          onClick={() =>
            setEditing({ key: '', isNew: true, titleEn: '', titleMr: '', bodyEn: '', bodyMr: '' })
          }
        >
      ...
      {editing && (
        <div className="mt-5 rounded-2xl border border-border bg-surface p-4 shadow-card">
          <div className="grid gap-3">
            <div>
              <Label htmlFor="cms-key" className={FIELD_LABEL}>
                {t('cmsKeyLabel')}
              </Label>
              <Input
                id="cms-key"
            ...
            <div>
              <Label htmlFor="cms-titleEn" className={FIELD_LABEL}>
                {t('titleEn')}
              </Label>
              <Input
                id="cms-titleEn"
            ...
            <div>
              <Label htmlFor="cms-titleMr" className={FIELD_LABEL}>
                {t('titleMr')}
              </Label>
              <Input
                id="cms-titleMr"
            ...
            <div>
              <Label htmlFor="cms-bodyEn" className={FIELD_LABEL}>
                {t('bodyEn')}
              </Label>
              <Textarea
                id="cms-bodyEn"
            ...
            <div>
              <Label htmlFor="cms-bodyMr" className={FIELD_LABEL}>
                {t('bodyMr')}
              </Label>
              <Textarea
                id="cms-bodyMr"
                value={editing.bodyMr}
                onChange={(e) => setEditing({ ...editing, bodyMr: e.target.value })}
                placeholder={t('bodyMr')}
                rows={4}
                className="rounded-xl border border-border bg-bg px-3 py-2 font-deva text-[14px] focus:border-accent"
              />
            </div>
            ...
            {list.data!.map((p) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={remove.isPending && remove.variables === p.key}
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() => {
                if (pendingDelete) remove.mutate(pendingDelete.key);
              }}
            >
              {t('delete')}
            </Button>
