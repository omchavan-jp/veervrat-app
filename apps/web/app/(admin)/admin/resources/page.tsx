  ...
  return (
    <div className="mx-auto max-w-[680px]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button className="shrink-0" onClick={() => setEditing({ ...empty })}><Plus className="h-4 w-4" /> {t('newResource')}</Button>
      </div>
      ...
      {editing && (
        <div className="mt-5 rounded-2xl border border-border bg-surface p-4 shadow-card">
          <div className="grid gap-3">
            <div>
              <Label htmlFor="resource-type" className={FIELD_LABEL}>{t('type')}</Label>
              <Select
                value={editing.type}
                onValueChange={(value) => { if (value) setEditing({ ...editing, type: value as 'FILE' | 'LINK' }); }}
              >
                <SelectTrigger id="resource-type" aria-label={t('type')} className="rounded-xl border-border bg-bg px-3 py-2 text-[14px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              <Label htmlFor="resource-title" className={FIELD_LABEL}>{t('title')}</Label>
              <Input id="resource-title" autoFocus value={editing.title} onChange={(e) => setEditing({ ...editing, title: e.target.value })} placeholder={t('title')} className="rounded-xl border border-border bg-bg px-3 py-2 text-[14px] focus:border-accent" />
            </div>
            <div>
              <Label htmlFor="resource-url" className={FIELD_LABEL}>{t('url')}</Label>
              <Input id="resource-url" value={editing.url} onChange={(e) => setEditing({ ...editing, url: e.target.value })} placeholder={t('url')} className="rounded-xl border border-border bg-bg px-3 py-2 text-[14px] focus:border-accent" />
            </div>
            <div>
              <Label htmlFor="resource-thumbnailUrl" className={FIELD_LABEL}>{t('thumbnailUrl')}</Label>
              <Input id="resource-thumbnailUrl" value={editing.thumbnailUrl} onChange={(e) => setEditing({ ...editing, thumbnailUrl: e.target.value })} placeholder={t('thumbnailUrl')} className="rounded-xl border border-border bg-bg px-3 py-2 text-[14px] focus:border-accent" />
            </div>
            <div>
              <Label htmlFor="resource-oneLiner" className={FIELD_LABEL}>{t('oneLiner')}</Label>
              <Input id="resource-oneLiner" value={editing.oneLiner} onChange={(e) => setEditing({ ...editing, oneLiner: e.target.value })} placeholder={t('oneLiner')} className="rounded-xl border border-border bg-bg px-3 py-2 text-[14px] focus:border-accent" />
            </div>
            <div>
              <Label htmlFor="resource-looseTags" className={FIELD_LABEL}>{t('looseTagsHint')}</Label>
              <Input id="resource-looseTags" value={editing.looseTags} onChange={(e) => setEditing({ ...editing, looseTags: e.target.value })} placeholder={t('looseTagsHint')} className="rounded-xl border border-border bg-bg px-3 py-2 text-[14px] focus:border-accent" />
            </div>
            <div className="flex gap-2">
              <Button onClick={() => save.mutate(editing)} disabled={!editing.title.trim() || save.isPending}>{save.isPending ? t('saving') : t('save')}</Button>
          ...
          <div className="space-y-2">
            {list.data!.items.map((r) => (
              <div key={r.id} className="flex items-center gap-3 rounded-2xl border border-border bg-surface p-4 shadow-card">
                ...
                <button onClick={() => startEdit(r)} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg text-muted transition-colors hover:bg-fg/[0.04] hover:text-fg" aria-label={t('edit')}><Pencil className="h-4 w-4" /></button>
                <button onClick={() => setPendingDelete(r)} disabled={remove.isPending && remove.variables === r.id} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg text-muted transition-colors hover:bg-danger/10 hover:text-danger disabled:opacity-50" aria-label={t('delete')}><Trash2 className="h-4 w-4" /></button>
              </div>
            ))}
          </div>
        )}
