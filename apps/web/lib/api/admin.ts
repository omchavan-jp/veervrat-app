...
// ─── Taxonomy ──────────────────────────────────────────────────────────────────
export type TaxonomyNode = { id: string; nameEn: string; nameMr: string | null; description: string | null };
...
export const adminApi = {
  // Taxonomy
  createVirtue: (b: VirtueInput) => api.post<Wrapped<TaxonomyNode>>('/admin/virtues', b).then((r) => r.data),
  updateVirtue: (id: string, b: Partial<VirtueInput>) =>
    api.patch<Wrapped<TaxonomyNode>>(`/admin/virtues/${id}`, b).then((r) => r.data),
  deleteVirtue: (id: string) => api.delete<Wrapped<{ id: string }>>(`/admin/virtues/${id}`).then((r) => r.data),

  createSubvirtue: (b: SubvirtueInput) => api.post<Wrapped<TaxonomyNode>>('/admin/subvirtues', b).then((r) => r.data),
  updateSubvirtue: (id: string, b: Partial<SubvirtueInput>) =>
    api.patch<Wrapped<TaxonomyNode>>(`/admin/subvirtues/${id}`, b).then((r) => r.data),
  deleteSubvirtue: (id: string) => api.delete<Wrapped<{ id: string }>>(`/admin/subvirtues/${id}`).then((r) => r.data),

  createWeakness: (b: WeaknessInput) => api.post<Wrapped<TaxonomyNode>>('/admin/weaknesses', b).then((r) => r.data),
  updateWeakness: (id: string, b: Partial<WeaknessInput>) =>
    api.patch<Wrapped<TaxonomyNode>>(`/admin/weaknesses/${id}`, b).then((r) => r.data),
  deleteWeakness: (id: string) => api.delete<Wrapped<{ id: string }>>(`/admin/weaknesses/${id}`).then((r) => r.data),

  // Shlokas
  createShloka: (b: ShlokaInput) => api.post<Wrapped<AdminShloka>>('/admin/shlokas', b).then((r) => r.data),
  updateShloka: (id: string, b: ShlokaInput) =>
    api.patch<Wrapped<AdminShloka>>(`/admin/shlokas/${id}`, b).then((r) => r.data),
  deleteShloka: (id: string) => api.delete<Wrapped<{ id: string }>>(`/admin/shlokas/${id}`).then((r) => r.data),

  // Scheduling + queue
  listSchedule: () => api.get<Wrapped<ScheduleEntry[]>>('/admin/shlokas/schedule').then((r) => r.data),
  schedule: (date: string, shlokaId: string) =>
    api.patch<Wrapped<ScheduleEntry>>('/admin/shlokas/schedule', { date, shlokaId }).then((r) => r.data),
  ...
  // Resources
  createResource: (b: ResourceInput) => api.post<Wrapped<{ id: string }>>('/admin/resources', b).then((r) => r.data),
  updateResource: (id: string, b: Partial<ResourceInput>) =>
    api.patch<Wrapped<{ id: string }>>(`/admin/resources/${id}`, b).then((r) => r.data),
  deleteResource: (id: string) => api.delete<Wrapped<{ id: string }>>(`/admin/resources/${id}`).then((r) => r.data),

  // Featured curation
  featureBlog: (id: string, featured: boolean) =>
    api.patch<Wrapped<{ id: string; featured: boolean }>>(`/moderation/blogs/${id}/featured`, { featured }).then((r) => r.data),
  featureExperience: (id: string, featured: boolean) =>
    api
      .patch<Wrapped<{ id: string; featured: boolean }>>(`/moderation/experiences/${id}/featured`, { featured })
      .then((r) => r.data),
};
