...
export const moderationApi = {
  getQueue: (cursor?: string) =>
    api
      .get<
        Wrapped<{ items: ModQueueItem[]; nextCursor: string | null }>
  ...
  approve: (id: string, edits?: CustomErcEdits) =>
    api
      .post<
        Wrapped<{ id: string; status: string; poolId: string }>
      >(`/moderation/custom-erc/${id}/approve`, edits ? { edits } : {})
      .then((r) => r.data),

  reject: (id: string, reason: string) =>
    api
      .post<
        Wrapped<{ id: string; status: string }>
      >(`/moderation/custom-erc/${id}/reject`, { reason })
      .then((r) => r.data),
};
