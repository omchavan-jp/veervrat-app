...
- **Verify with something that behaves like the thing you are claiming about.** A check run with
  ...
  experience. Eight instances, each green everywhere and broken in use:
  ...
  | Every test granting `VRATMITRA` in a fixture | the flow, given the role | **nobody could get the role** — accept returned 403 to every real user for months |
  ...
  The last row is the worst kind, and it is worth its own rule. **A fixture may grant what the
  product cannot, and nothing else.** `e2e/helpers/global-setup.ts` may insert `admin` and
  `moderator` because no product path grants them. It may not insert anything a user is supposed
  to obtain themselves: doing so seeds a state **no user can reach**, and every test downstream
  then proves something about a world that does not exist. Full account:
  `ops/audit/README.md`.
