...
test.describe('Flow 3: VM invite → accept → submit → VM approves', () => {
  ...
  test('VA starts a journey and invites a journey VM, who accepts', async () => {
    ...
    journeyId = (
      await (
        await ctx.post('/api/v1/journeys', {
          ...
          data: { sentenceId, weaknessId, title: 'E2E Flow 3 Journey' },
        })
      ).json()
    ).data.id;
    const pool = (await (await ctx.get(`/api/v1/journeys/${journeyId}/resolutions/pool`)).json())
      .data as { id: string }[];
    resolutionId = (
      await (
        await ctx.post(`/api/v1/journeys/${journeyId}/resolutions`, {
          headers: apiHeaders(csrf),
          data: { poolItemId: pool[0].id },
        })
      ).json()
    ).data.id;
    ...
    const accept = await vm.ctx.post(`/api/v1/invitations/${token}/accept`, {
  ...
  test('VA submits the ERC for closure and the assigned VM approves it', async () => {
    ...
    await va2.ctx.patch(`/api/v1/journeys/${journeyId}/resolutions/${resolutionId}/status`, {
      headers: apiHeaders(va2.csrf),
      data: { status: 'in_progress' },
    });
    const submit = await va2.ctx.patch(
      `/api/v1/journeys/${journeyId}/resolutions/${resolutionId}/status`,
      { headers: apiHeaders(va2.csrf), data: { status: 'submitted' } },
    );
    expect(
      submit.ok(),
      `submit for closure: ${submit.status()} ${await submit.text()}`,
    ).toBeTruthy();
    await va2.ctx.dispose();

    // VM: approve the submitted item (scoped approval — assigned journey VM).
    const vm = await loginApi(VM);
    const approve = await vm.ctx.post(
      `/api/v1/journeys/${journeyId}/resolutions/${resolutionId}/approve`,
      { headers: apiHeaders(vm.csrf) },
    );
