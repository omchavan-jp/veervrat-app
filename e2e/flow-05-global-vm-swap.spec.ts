...
import { deleteUserByEmail, latestInvitationToken, scalar, lit, sampleSentenceWeakness } from './helpers/db';
...
test.describe('Flow 5: global VM swap → migration', () => {
  ...
  test('VA with a global VM (also a journey VM) removes them with unassign cascade', async () => {
    ...
    // Journey for the VA.
    journeyId = (await (await ctx.post('/api/v1/journeys', { headers: apiHeaders(csrf), data: { sentenceId, weaknessId, title: 'E2E Flow 5 Journey' } })).json()).data.id;

    // Invite the shared VM both globally and to the journey.
    await ctx.post('/api/v1/invitations', { headers: apiHeaders(csrf), data: { type: 'VM_GLOBAL', inviteeUsername: VM.username } });
    const globalToken = latestInvitationToken(VM.email)!;
    await ctx.post('/api/v1/invitations', { headers: apiHeaders(csrf), data: { type: 'VM_JOURNEY', inviteeUsername: VM.username, scopeId: journeyId } });
    await ctx.dispose();

    // VM accepts both invitations (global first, then journey — latestInvitationToken returns newest).
    let vm = await loginApi(VM);
    await vm.ctx.post(`/api/v1/invitations/${globalToken}/accept`, { headers: apiHeaders(vm.csrf) });
    ...
    await vm.ctx.post(`/api/v1/invitations/${journeyToken}/accept`, { headers: apiHeaders(vm.csrf) });
    await vm.ctx.dispose();
    ...
    expect(scalar(`SELECT count(*) FROM vm_relationships WHERE vratarthi_id=${lit(vaId)} AND vm_id=${lit(vmId)} AND ended_at IS NULL`)).toBe('1');
    expect(scalar(`SELECT count(*) FROM journey_vm_assignments WHERE journey_id=${lit(journeyId)} AND vm_id=${lit(vmId)} AND ended_at IS NULL`)).toBe('1');

    // --- The swap: remove the global VM with the unassign cascade (Item 37) ---
    const va2 = await loginApi(va);
    const remove = await va2.ctx.delete('/api/v1/vm-relationships/global', { headers: apiHeaders(va2.csrf), data: { cascade: 'unassign' } });
    ...
    expect(body.affectedJourneys.some((j: { journeyId: string }) => j.journeyId === journeyId)).toBeTruthy();
    await va2.ctx.dispose();

    // Both relationships are now ended (the migration removed them).
    expect(scalar(`SELECT count(*) FROM vm_relationships WHERE vratarthi_id=${lit(vaId)} AND vm_id=${lit(vmId)} AND ended_at IS NULL`)).toBe('0');
    expect(scalar(`SELECT count(*) FROM journey_vm_assignments WHERE journey_id=${lit(journeyId)} AND vm_id=${lit(vmId)} AND ended_at IS NULL`)).toBe('0');
  });
  ...
  test('keep cascade leaves journey assignments intact', async () => {
    ...
    const va2 = await loginApi(va);
    const remove = await va2.ctx.delete('/api/v1/vm-relationships/global', { headers: apiHeaders(va2.csrf), data: { cascade: 'keep' } });
