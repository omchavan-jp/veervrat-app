...
- [ ] 1.1 Create `apps/api/src/modules/journeys/journeys.repository.ts`:
...
- [ ] 2.1 Add `JourneyConflictException` to `apps/api/src/common/exceptions/app.exceptions.ts` (409, error: `JOURNEY_ALREADY_EXISTS`, includes `existingJourneyId` in details)
- [ ] 2.2 Add `InvalidStateTransitionException` to `apps/api/src/common/exceptions/app.exceptions.ts` (409, error: `INVALID_STATE_TRANSITION`)
- [ ] 2.3 Create `apps/api/src/modules/journeys/dto/create-journey.dto.ts` — `sentenceId: string (IsUUID)`, `weaknessId: string (IsUUID)`, `title?: string (IsOptional, IsString, MaxLength 255)`
- [ ] 2.4 Create `apps/api/src/modules/journeys/dto/update-journey-state.dto.ts` — `action: 'pause' | 'resume' (IsIn)`
- [ ] 2.5 Create `apps/api/src/modules/journeys/dto/update-journey-title.dto.ts` — `title: string (IsString, MinLength 1, MaxLength 255)`

## 3. Backend — Service

- [ ] 3.1 Create `apps/api/src/modules/journeys/journeys.service.ts`:
...
- [ ] 4.1 Create `apps/api/src/modules/journeys/journeys.controller.ts`:
...
- [ ] 4.2 Create `apps/api/src/modules/journeys/journeys.module.ts` — imports AuthModule, exports JourneysService
- [ ] 4.3 Register `JourneysModule` in `apps/api/src/app.module.ts`

## 5. Backend — Tests

- [ ] 5.1 Create `apps/api/src/modules/journeys/journeys.service.spec.ts`:
...
- [ ] 6.1 Create `apps/web/lib/api/journeys.ts` — typed functions and response types:
...
- [ ] 6.2 Create `apps/web/hooks/use-journeys.ts`:
...
- [ ] 7.1 Create `apps/web/app/(app)/journeys/new/page.tsx` — `'use client'`; reads `sentenceId` + `weaknessId` from `useSearchParams()`; calls `useCreateJourney()` on mount; on 409 redirects to existing journey; on missing params redirects to `/study`; shows spinner while pending
- [ ] 7.2 Create `apps/web/app/(app)/journeys/page.tsx` — `'use client'`; uses `useJourneys()`; renders journey cards (title, sentence excerpt, state badge, weakness tags, last updated); empty state with link to `/study`
- [ ] 7.3 Create `apps/web/app/(app)/journeys/[id]/page.tsx` — `'use client'`; uses `useJourney(id)`:
...
- [ ] 8.1 Add `journey` namespace to `apps/web/messages/en.json`: keys for list (title, emptyState, emptyStateCta), detail (cultivating, noVm, pause, resume, statusOverview, emptyState, emptyStateCta, tabExposures, tabResolutions, tabChallenges, tabChat), stateBadges (active, paused, dormant, completed)
- [ ] 8.2 Add matching `journey` namespace to `apps/web/messages/mr.json`
- [ ] 8.3 Add "Journeys" nav link to `apps/web/components/layout/header.tsx`

## 9. Frontend — Tests

- [ ] 9.1 Create `apps/web/src/test/journey-list.test.tsx`:
...
- [ ] 9.2 Run `pnpm test` in both apps — all tests must pass before apply is marked done
